...
terraform {
  ...
  before_hook "importNetwork" {
    #gcdocsinstallers
    commands     = ["apply", "plan"]
    #send to import powershell to intelegently import
    execute = ["PowerShell", "tfImport/tfImport.ps1", "-SubscriptionID", local.subscription_id, "-ResourceGroupName", local.resource_group_name, "-environment", local.environment]
  }
}
